...
public abstract class ValueReference<TValue, TAsset> where TAsset : ValueAssetSO<TValue>
{
    ...
    [HideIf("useValue", Animate = false), HorizontalGroup("Reference"), OnValueChanged(nameof(UpdateAsset)), HideLabel][SerializeField] 
    ...
    [ShowIf("@assetReference != null && useValue == false"), EnableIf("editAsset")][InlineEditor(InlineEditorObjectFieldModes.Hidden)][SerializeField] 
    protected TAsset _assetReference; // inline attribute to be editable
